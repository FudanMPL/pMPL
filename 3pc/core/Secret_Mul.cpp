...
void Secret_Mul::Mul_truncation(Matrixint64 &z)
{
    int row = z.rows();
    int col = z.cols();
    Matrixint64 r0(row, col), r1(row, col), r2(row, col), r3(row, col);
    Matrixint64 r_0(row, col), r_1(row, col), r_2(row, col), r_3(row, col);
    ...
    {
        Matrixint64 z1_sub_r1 = z - r2;
        Secret_Mul::reveal(z1_sub_r1);
        z = r_2;
    }
    else if (party == 3)
    {
        Matrixint64 z2_sub_r2 = z - r3;
...
Matrixint64 Secret_Mul::Multiply(Matrixint64 &x, Matrixint64 &y, Matrixint64 &r, Matrixint64 &q, Matrixint64 &t)
{
    ...
    {
        Matrixint64 temp;
        if (x.rows() == B && y.cols() == 1)
        {
            temp = Secret_Mul::m_01;
        }
        else if (x.rows() == D && y.cols() == 1)
        {
            temp = Secret_Mul::m_11;
        }
        else if (x.rows() == B && y.cols() == B)
        {
            temp = Secret_Mul::m_21;
        }
        else if (x.rows() == D && y.cols() == B)
        {
...
Matrixint64 Secret_Mul::reveal2(Matrixint64 &data)
{
    int row = data.rows();
    int col = data.cols();
    Matrixint64 res;
